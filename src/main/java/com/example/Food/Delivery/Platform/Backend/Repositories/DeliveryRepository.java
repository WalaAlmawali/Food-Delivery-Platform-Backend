...
public interface DeliveryRepository extends JpaRepository<Delivery, Integer> {
    ...
    @Query("SELECT d FROM Delivery d WHERE d.deliveryDriver.id = :driverId AND d.status <> com.yourpackage.OrderStatus.DELIVERED AND d.isActive = true")
    List<Delivery> findActiveDeliveryByDriver(@Param("driverId") Integer driverId);
    ...
    @Query("SELECT COUNT(d) FROM Delivery d WHERE d.deliveryDriver.id = :driverId AND d.status = com.yourpackage.OrderStatus.DELIVERED AND d.isActive = true")
    Long countCompletedDeliveriesByDriver(@Param("driverId") Integer driverId);
}
